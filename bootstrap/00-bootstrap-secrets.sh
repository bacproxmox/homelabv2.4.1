...
echo
echo "🔐 Homelab v2.4.1 - secrets/env bootstrap"
...
if [[ ! -f "$SECRETS_DIR/global.env" ]]; then
  ...
  {
    write_env_header
    write_env_line HOMELAB_VERSION "2.4.1"
...
if [[ ! -f "$SECRETS_DIR/chia-bootstrap.env" ]]; then
  ...
  case "$CHIA_DB_CHOICE" in
    ...
    4)
      ...
      ask_text_default CHIA_DB_MANUAL_PATH "VM107 üzerindeki DB dosyası path'i" "/home/bacmaster/chia-db-download/blockchain_v2_mainnet.sqlite"
  ...
  esac
  {
    ...
    write_env_line CHIA_DB_DOWNLOAD_DIR "/home/bacmaster/chia-db-download"
